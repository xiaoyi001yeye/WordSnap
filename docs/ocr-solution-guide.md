...
4. 应用把 JSON 解析成“单词 + 词性/释义”的词条，再进入考试流程；音标由固定词典 API 补齐
...
- 词书条目识别需要同时理解单词、词性和中文释义，单独文本 OCR 容易拆散上下文
...
- 优先把一条词书内容识别成一个词条
- 不输出音标；音标统一由应用的固定词典 API 获取
- `part_of_speech` 使用对象数组，每个词性对应一个中文释义
- 图片中的词性和释义只作为线索，允许大模型鉴别、纠错和补充，最终以模型生成的词性和释义为准
...
- `word`
- `meaning`，由 `part_of_speech` 中的词性和释义合并而来
- `confidence`
