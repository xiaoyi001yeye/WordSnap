...
- 对识别结果做英文单词抽取
- 应用内默认连接 PaddleOCR 服务，不再向用户暴露地址配置
...
- 构建时通过 `--dart-define=PADDLE_OCR_ENDPOINT=...` 指定的统一地址
- Android 模拟器：`http://10.0.2.2:8080/ocr`
- 本机回环地址：`http://127.0.0.1:8080/ocr`
- 本机回环别名：`http://localhost:8080/ocr`
...
真机调试时如果需要接到电脑上的服务，不再在应用里手动填写地址，而是应该在构建时直接指定统一地址。不要使用：
...
应该在启动应用时传入电脑在同一局域网中的地址，例如：

- `--dart-define=PADDLE_OCR_ENDPOINT=http://192.168.1.10:8080/ocr`
